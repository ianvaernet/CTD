...
public class DentistDAOH2 implements IDAO<Dentist> {
    ...
    public DentistDAOH2(Connection dbConnection) {
    ...
    public int create(Dentist newDentist) {
        ...
        try {
            ...
            if (generatedKeys.next()) {
                logger.debug("Odontólogo creado: " + newDentist.toString());
    ...
    public Dentist update(int id, Dentist updatedDentist) {
        logger.debug("Actualizando odontólogo con ID=" + id + "...");
        try {
            ...
            if (affectedRows > 0) {
                logger.debug("Odontólogo actualizado: " + updatedDentist);
    ...
    public void delete(int id) {
        ...
        try {
            ...
            if (affectedRows == 0) {
                ...
                logger.debug("Odontólogo eliminado");
            }
        } catch (SQLException e) {
            logger.error(e.getMessage());
            e.printStackTrace();
    ...
    public Dentist get(int id) {
        logger.debug("Obteniendo odontólogo con ID=" + id + "...");
        ...
        try {
            ...
            if (result.next()) {
                ...
                logger.debug("Odontólogo encontrado: " + dentist);
            } else {
                logger.warn("No existe ningún odontólogo con el ID proporcionado");
            }
        } catch (SQLException e) {
